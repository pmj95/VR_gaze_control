...
Um in eine \ac{VR}-Welt einzutauchen, wird ein \ac{VR}-Headset benötigt. Im Rahmen dieser Arbeit wird das \acs{VR}-Headset HTC Vive verwendet. Das Headset wurde von HTC gemeinsam mit dem Spieleentwickler Valve entwickelt und ist seit dem Jahr 2016 auf dem Markt erhältlich. Das \acs{VR}-Headset lässt sich in das \ac{HMD}, Controller und in das Tracking System unterteilen.

\subsection{\acl{HMD}}
%Das \ac{HMD} ist die Hauptkomponente, mit welcher der Benutzer in die \ac{VR}-Umgebung eintaucht. Durch Aufsetzen eines \ac{HMD}
%In \autoref{fig:vive-hardware-hmd-1} ist das \ac{HMD} der HTC Vive dargestellt. 
