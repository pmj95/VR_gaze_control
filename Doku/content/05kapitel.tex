...
\todo[inline]{JH: Knopf, Button oder Bedienelement. Auf eines dieser Wörter müssen wir uns einigen}
...
Die beiden variablen Parameter in Fitts' Gesetz sind die Distanz D und die Breite W (vgl. \autoref{section:FittsLaw}). Die Breite W wird in den Versuchen mit der Veränderung der Knopfgröße erreicht\todo{JH: Also iwie macht der Satz so kein Sinn. :D Vlt eher: Die Breite W beschreibt...}, die Distanz D wird über verschiedene, pseudo-zufällige\todo{JH: Naja ist in 3DLevel nicht pseudozufällig}, Muster variiert (vgl. \autoref{section:game} \todo{JH: Stehen lassen bis Kapitel 4 fertig!!!!!!!!!!!!!!!!!!! haben wir sowas schon irgendwo? JH: Bis jetzt noch nicht wird aber denk in Game erklärt}). In \autoref{fig:plotbuttons} ist der Einfluss der unterschiedlichen Distanzen zwischen zwei Knöpfen auf die Zeit zwischen der Aktivierung des ersten und des zweiten Knopfes dargestellt\todo{JH: Mir ist klar was du meinst, ABER es liest sich so, dass es um die Zeit geht zwischen dem aller ersten und dem zweiten Knopf und drei, vier usw. nicht darestellt sind.}. Die durchgezogenen Linien zeigen die tatsächlichen Werte, die gestrichelten Linien zeigen den Trend der Werte an. Damit wird überprüft, ob eine unterschiedliche Distanz zwischen Knöpfen auch zu einer unterschiedlichen Aktivierungszeit führt. Für die Daten wurden alle erfolgreichen Eingaben der Nutzer mit der Zeit und dem entsprechenden Knopf aufgezeichnet und analysiert. Daraus lässt sich die Distanz zwischen den Knöpfen und die benötigte Zeit berechnen. Da jeder Nutzer jeden Versuchsaufbau ein mal durchläuft, gibt es zu jeder Distanz mehrere Zeitwerte. Aus diesen Werten wird der Durchschnitt in \autoref{fig:plotbuttons} pro Versuchsaufbau dargestellt\todo{Erklärung so okay? versteht man das? JH: denke schon}. Durch die geringe Probandenzahlen sind die Zahlen nur bedingt aussagekräftig, da die Distanz nur einer der Faktoren ist, die die \ac{MT} (vgl. \autoref{section:FittsLaw}) beeinflusst. Andere Faktoren, wie die Größe der Knöpfe oder die Erfahrung des Probanden mit Eye-Tracking und VR kann diese Werte deutlich stärker beeinflussen, als die Distanz alleine. Bei einer großen Zahl Probanden ist es wahrscheinlicher, dass sich diese Störfaktoren gegenseitig aufheben und so ein besserer Zusammenhang zwischen Distanz und Zeit erkennbar wird. 
\todo{JH: \autoref{fig:plotbuttons} ziemlich unglückliche Darstellung. Während Big je blau ist ist small orange und gelb (oder iwie was derartiges).}
\todo{JH: \autoref{fig:plotbuttons} in Anhang schieben wenn sie eine Seite komplett einnimmt?}
\begin{figure}[!htbp]
	\centering
...
Zur Auswertung der Blickdaten werden die, wie in \autoref{section:measurement} beschrieben aufgezeichnet \todo{JH: Da stimmt was nicht}wurden, analysiert \todo{JH: Komma??} um eine grafische Darstellung zu ermöglichen. In \autoref{fig:plotheatmapprob2} sind die Blickdaten von Proband 2 aus den Versuchen mit den Steuermethoden Eye-Tracking/Trigger und Eye-Tracking/Blinzeln mit jeweils großen und kleinen Knöpfen dargestellt. Im \autoref{appendix:heatmaps} sind die Heatmaps aller Probanden zu sehen. In diesem Abschnitt werden die Blickdaten anhand von Proband 2 näher betrachtet.  
\todo{JH: \autoref{fig:plotheatmapprob2} ist doch schon im Anhang vorhanden. Verweise doch direkt darauf. Nimmt eh eine komplette Seite ein...}
\begin{figure}[!htbp]
	\centering
...
Die Blickdaten starten meist \todo{JH: interessant das genau einer iwo anders anfängt} oben rechts, da sich an dieser Stelle der Go Button befindet. Der Blick des Probanden wechselt danach immer \todo{JH: Ist es wirklich so? Immer ist schon ein gefährliches Wort. Und lässt sich anhand der Heatmap auch nicht belegen} zwischen dem auszuwählenden Knopf und der Anzeige oben\todo{JH: Der rest ist obsolet. Wird ja eig schon in Kapitel 4 erklärt}, an der angezeigt wird, welcher Knopf als nächstes ausgewählt werden muss. Deshalb sind oft große Sprünge von einem Knopf in Richtung des Wortes \grqq game\grqq{} zu sehen. Je höher die Anzahl an Strichen um einen Knopf ist \todo{JH: Also ich weiß was du meinst aber iwie ist das nicht ganz klar}, desto länger hat der Proband seinen Blick auf diesen Knopf gerichtet. Dies ist vor allem bei der Steuermethode Eye-Tracking/Blinzeln mit kleinen Knöpfen bei dem Knopf 1 zu sehen. Hier hat der Nutzer sehr lange seinen Blick auf diesen Knopf gerichtet. Das liegt daran, dass beim Blinzeln der Eye-Tracker kurzzeitig verrutscht und somit nicht mehr über dem Knopf liegt \todo{JH: Die Aussage ist doch falsch. Der Eyetracker kann nicht verrutschen, da fest verbaut in der VR Brille. Was verrutscht sind die Gaze data}. Dadurch kommt es zu einer fehlerhaften Eingabe und der Nutzer muss erneut probieren, den Knopf zu treffen. Proband 2 war bei diesem Versuchsaufbau am schnellsten, weshalb hier im Vergleich zu den anderen Probanden (\autoref{appendix:heatmaps}) deutlich weniger Striche zu sehen sind. Bei den Blickdaten ist sehr gut die Ungenauigkeit, die durch vermehrtes Blinzeln auftritt, zu erkennen. 
\todo{reicht das zur Beschreibung? mir fällt sonst nichts ein JH: Denke schon}
